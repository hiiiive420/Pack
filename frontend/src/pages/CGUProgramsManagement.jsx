...
const CGUProgramsManagement = () => {
  const { user, logout } = useContext(AuthContext);

  // 🔐 Permission Guard
  if (
    user.role !== "ADMIN" &&
    !user.permissions.includes("CGU_PROGRAMS_MANAGEMENT")
  ) {
  ...
  const submit = async () => {
    ...
    if (editingId) {
      await api.put(`/cgu-programs/${editingId}`, form);
      alert("CGU Program record updated");
    } else {
      await api.post("/cgu-programs", form);
      alert("CGU Program record added");
    }
  ...
  const deleteRecord = async (id) => {
    ...
    await api.delete(`/cgu-programs/${id}`);
    loadAll();
  };

  // ---------------- UI ----------------
  return (
    <DashboardLayout user={user} logout={logout} title="CGU Programs Management">
      <div className="page-container " style={{ fontFamily: "'Inter', sans-serif" }}>
        <h2 className="page-title">Career Guidance Unit (CGU) Programs</h2>
        ...
        <div className="card">
          ...
          <div className="form-grid-4">
            ...
            <div>
              ...
              <input value={form.programmeName} disabled style={{ backgroundColor: "#f1f5f9" }} />
            </div>
          ...
          <button className="primary-btn" onClick={submit} style={{ marginTop: 20 }}>
            {editingId ? "Update Program Record" : "Add Program Record"}
          </button>
        </div>

        {/* DATA TABLE CARD */}
        <div className="card">
          <h3 style={{ marginBottom: 20 }}>CGU Expenditure Ledger</h3>
...
export default CGUProgramsManagement;